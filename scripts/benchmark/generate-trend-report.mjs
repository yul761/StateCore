...
const md = [
  ...
  ...benchmarks.map((item) =>
    [
      ...
      `- Retrieve mode: ${item.retrieveMode}, retrieve limit ${item.retrieveLimit}, hit ${item.retrieveHitRate}, strict ${item.retrieveStrictHitRate}, embedding requested ${item.retrieveEmbeddingRequested ? "yes" : "no"}, configured ${item.retrieveEmbeddingConfigured ? "yes" : "no"}, candidate limit ${item.retrieveEmbeddingCandidateLimit ?? "default"}`,
