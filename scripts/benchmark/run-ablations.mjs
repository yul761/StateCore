...
function collectSummary(jsonPath) {
  ...
  return {
    ...
    runtimeEvidenceStateConfidenceRate: data.metrics?.runtime?.evidenceStateConfidenceRate ?? 0,
    runtimeEvidenceStateTransitionTaxonomyRate: data.metrics?.runtime?.evidenceStateTransitionTaxonomyRate ?? 0,
    ...
    answerEvidenceStateConfidenceRate: data.metrics?.answer?.evidenceStateConfidenceRate ?? 0,
    answerEvidenceStateTransitionTaxonomyRate: data.metrics?.answer?.evidenceStateTransitionTaxonomyRate ?? 0,
...
function compareAgainstBaseline(baseline, item) {
  return {
    ...
    openQuestionContinuityRate: roundDelta((item.openQuestionContinuityRate ?? 0) - (baseline.openQuestionContinuityRate ?? 0)),
    riskContinuityRate: roundDelta((item.riskContinuityRate ?? 0) - (baseline.riskContinuityRate ?? 0)),
    resolvedOpenQuestionIntrusionRate: roundDelta((item.resolvedOpenQuestionIntrusionRate ?? 0) - (baseline.resolvedOpenQuestionIntrusionRate ?? 0)),
    resolvedRiskIntrusionRate: roundDelta((item.resolvedRiskIntrusionRate ?? 0) - (baseline.resolvedRiskIntrusionRate ?? 0)),
    ...
    groundedResponseStateConfidenceRate: roundDelta((item.groundedResponseStateConfidenceRate ?? 0) - (baseline.groundedResponseStateConfidenceRate ?? 0)),
    groundedResponseStateTransitionTaxonomyRate: roundDelta((item.groundedResponseStateTransitionTaxonomyRate ?? 0) - (baseline.groundedResponseStateTransitionTaxonomyRate ?? 0)),
    ...
    runtimeEvidenceStateConfidenceRate: roundDelta((item.runtimeEvidenceStateConfidenceRate ?? 0) - (baseline.runtimeEvidenceStateConfidenceRate ?? 0)),
    runtimeEvidenceStateTransitionTaxonomyRate: roundDelta((item.runtimeEvidenceStateTransitionTaxonomyRate ?? 0) - (baseline.runtimeEvidenceStateTransitionTaxonomyRate ?? 0)),
    ...
    replayStabilityBlend: roundDelta((item.replayStabilityBlend ?? 0) - (baseline.replayStabilityBlend ?? 0))
  };
}
...
function summarizeDeltas(cases) {
  ...
  if (!compared.length) {
    return {
      ...
      bestWorkingNoteContinuity: null,
      worstWorkingNoteContinuity: null,
      bestResolvedWorkingNoteIntrusion: null,
      worstResolvedWorkingNoteIntrusion: null,
  ...
  return {
    ...
    bestWorkingNoteContinuity: byMetric("openQuestionContinuityRate", "desc"),
    worstWorkingNoteContinuity: byMetric("openQuestionContinuityRate", "asc"),
    bestResolvedWorkingNoteIntrusion: byMetric("resolvedRiskIntrusionRate", "asc"),
    worstResolvedWorkingNoteIntrusion: byMetric("resolvedRiskIntrusionRate", "desc"),
...
const lines = [
  ...
  formatDeltaEntry("Best working-note continuity delta", deltaSummary?.bestWorkingNoteContinuity, "openQuestionContinuityRate"),
  formatDeltaEntry("Worst working-note continuity delta", deltaSummary?.worstWorkingNoteContinuity, "openQuestionContinuityRate"),
  formatDeltaEntry("Best resolved working-note intrusion delta", deltaSummary?.bestResolvedWorkingNoteIntrusion, "resolvedRiskIntrusionRate"),
  formatDeltaEntry("Worst resolved working-note intrusion delta", deltaSummary?.worstResolvedWorkingNoteIntrusion, "resolvedRiskIntrusionRate"),
  ...
  ...summaries.map(
    (s) => [
      ...
      `- Digest quality: consistency ${s.digestConsistencyPassRate}, repeatability ${s.digestRepeatabilityRate}, omission-warning ${s.digestOmissionWarningRate}, temporary-todo-intrusion ${s.temporaryTodoIntrusionRate}, superseded-doc-intrusion ${s.supersededDocumentIntrusionRate}`,
      ...
      `- Working-note retention: open-questions ${s.openQuestionContinuityRate}, risks ${s.riskContinuityRate}, state open-questions ${s.stateOpenQuestionContinuityRate}, state risks ${s.stateRiskContinuityRate}`,
      `- Working-note intrusion: resolved open-questions digest ${s.resolvedOpenQuestionIntrusionRate}, resolved open-questions state ${s.stateResolvedOpenQuestionIntrusionRate}, resolved risks digest ${s.resolvedRiskIntrusionRate}, resolved risks state ${s.stateResolvedRiskIntrusionRate}`,
      ...
      `- Runtime evidence detail: digest-summary ${s.runtimeEvidenceDigestSummaryRate}, event-snippet ${s.runtimeEvidenceEventSnippetRate}, ranking-reason ${s.runtimeEvidenceEventRankingReasonRate}, score ${s.runtimeEvidenceEventScoreRate}, embedding-reason ${s.runtimeEvidenceEventEmbeddingReasonRate}, document-source ${s.runtimeEvidenceEventDocumentSourceRate}, state-summary ${s.runtimeEvidenceStateSummaryRate}, state-provenance ${s.runtimeEvidenceStateProvenanceRate}, state-transition-taxonomy ${s.runtimeEvidenceStateTransitionTaxonomyRate}, recent-state-changes ${s.runtimeEvidenceRecentStateChangesRate}`,
      `- Runtime state evidence: confidence ${s.runtimeEvidenceStateConfidenceRate}, state-summary ${s.runtimeEvidenceStateSummaryRate}, state-provenance ${s.runtimeEvidenceStateProvenanceRate}, state-transition-taxonomy ${s.runtimeEvidenceStateTransitionTaxonomyRate}, recent-state-changes ${s.runtimeEvidenceRecentStateChangesRate}`,
      `- Grounded response view: success ${s.groundedResponseSuccessRate}, evidence ${s.groundedResponseEvidenceCoverageRate}, ranking-reason ${s.groundedResponseRankingReasonRate}, score ${s.groundedResponseEventScoreRate}, state-summary ${s.groundedResponseStateSummaryRate}, state-confidence ${s.groundedResponseStateConfidenceRate}, state-transition-taxonomy ${s.groundedResponseStateTransitionTaxonomyRate}`,
      `- Answer grounding: ${s.answerSuccess}/${s.answerRuns} success, evidence ${s.answerEvidenceCoverageRate}, ranking-reason ${s.answerEvidenceEventRankingReasonRate}, score ${s.answerEvidenceEventScoreRate}, state-summary ${s.answerEvidenceStateSummaryRate}, state-confidence ${s.answerEvidenceStateConfidenceRate}, state-transition-taxonomy ${s.answerEvidenceStateTransitionTaxonomyRate}`,
      `- Replay transitions: match ${s.replayTransitionTaxonomyMatchRate}, cross-run divergence ${s.replayCrossRunTransitionDivergenceRate}, stability blend ${s.replayStabilityBlend}`,
