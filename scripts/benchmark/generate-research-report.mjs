...
const lines = [
  ...
  `- Retrieve metrics: mode ${benchmark.metrics?.retrieve?.mode ?? "heuristic"}, retrieve limit ${benchmark.metrics?.retrieve?.limit ?? 20}, hit ${benchmark.metrics?.retrieve?.hitRate ?? 0}, strict ${benchmark.metrics?.retrieve?.strictHitRate ?? 0}, embedding requested ${benchmark.metrics?.retrieve?.embeddingRequested ? "yes" : "no"}, embedding configured ${benchmark.metrics?.retrieve?.embeddingConfigured ? "yes" : "no"}, candidate limit ${benchmark.metrics?.retrieve?.embeddingCandidateLimit ?? "default"}, embedding model ${benchmark.metrics?.retrieve?.embeddingModel || "none"}`,
